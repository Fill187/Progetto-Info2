public class Stazione extends Casella{
    protected int prezzo;
    protected int affitto;
    protected Giocatore proprietario;
    
    public Stazione(String nome, int posizione, int prezzo, int affitto, Giocatore proprietario){
        super(nome, posizione);
        this.prezzo = prezzo;
        this.affitto = affitto;
        this.proprietario = null;
    }
    public int getPrezzo(){
        return prezzo;
    }
    public void setPrezzo(int prezzo){
        this.prezzo = prezzo;
    }
    public int getAffitto(){
        return affitto;
    }
    public void setAffitto(Giocatore g){
        switch(g.getStazioniPossedute()){
            case 1:
                this.affitto = 25;
                break;
            case 2:
                this.affitto = 50;
                break;
            case 3:
                this.affitto = 100;
                break;
            case 4:
                this.affitto = 200;
                break;
        }
    }
    public Giocatore getProprietario(){
        return proprietario;
    }
    public void setProprietario(Giocatore proprietario){
        this.proprietario = proprietario;
    }
    public String toString(){
        String s = "";
        s += "Nome: " + nome + "\n";
        s += "Posizione" + posizione + "\n";
        s += "Affitto: " + affitto + "\n";
        s += "Prezzo: " + prezzo + "\n";
        s += "Proprietario: " + proprietario + "\n";
        return s;
    }
}
